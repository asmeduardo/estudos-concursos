...
function fmtMinutes(minutes: number): string { return fmtSeconds(Math.max(0, minutes) * 60); }
...
function renderHud(list = ranked()): void {
  ...
  $('#hudTime').textContent = fmtSeconds(seconds); $('#hudTarget').textContent = `${activeContest().targetAccuracy}% · ${fmtMinutes(state.targetMinutes)}`; $('#hudRecovery').textContent = String(recovery.length); $('#hudRecovery').style.color = recovery.length ? '#fb7185' : '#34d399'; $('#hudNext').textContent = next ? next.name : 'Aguardando dados'; $('#hudNext').title = next ? next.d.action : 'Aguardando sincronização das plataformas'; $('#hudSync').textContent = state.sync ? fmtDate(state.sync.at) : 'local';
}
...
function pauseTimer(reason = 'manual'): void { if (reason === 'manual' || reason === 'hud') manualPause = true; if (!timerRunning) return; timerLoop(); timerRunning = false; if (timerTick) window.clearInterval(timerTick); timerTick = null; checkpointTimer(reason, true); render(); }
