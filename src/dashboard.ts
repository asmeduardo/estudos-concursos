...
function focused(): boolean { return document.visibilityState === 'visible' && document.hasFocus(); }
function timerLoop(): void { if (!timerRunning) return; const now = Date.now(); if (focused() && timerLast) pendingSessionSeconds += Math.max(0, Math.min(5, (now - timerLast) / 1000)); timerLast = now; renderTimer(); }
function startTimer(): void { if (timerRunning) return; timerRunning = true; timerLast = Date.now(); timerTick = window.setInterval(timerLoop, 1000); renderTimer(); }
function pauseTimer(reason = 'manual'): void { if (!timerRunning) return; timerLoop(); timerRunning = false; if (timerTick) window.clearInterval(timerTick); timerTick = null; if (pendingSessionSeconds >= 1) { currentDay().seconds += Math.round(pendingSessionSeconds); currentDay().sessions.push({ type: $<HTMLSelectElement>('#sessionType').value, seconds: Math.round(pendingSessionSeconds), endedAt: new Date().toISOString(), reason }); pendingSessionSeconds = 0; saveState(); } render(); }
...
async function pullLocalSnapshot(): Promise<boolean> { try { const response = await fetch('http://127.0.0.1:8765/tec_sync.json', { cache: 'no-store' }); if (!response.ok) throw new Error(`HTTP ${response.status}`); importRecords(await response.json(), 'ponte local'); return true; } catch (error) { $('#syncMessage').innerHTML = `<strong>Ponte local indisponível.</strong> ${esc(error instanceof Error ? error.message : error)}.`; return false; } }
function exportData(): void { const blob = new Blob([JSON.stringify({ version: 1, exportedAt: new Date().toISOString(), ...state }, null, 2)], { type: 'application/json' }); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = `dataprev-estudo-${today}.json`; link.click(); URL.revokeObjectURL(link.href); }
...
$('#export').addEventListener('click', exportData); $('#tecFile').addEventListener('change', (event) => { const file = (event.target as HTMLInputElement).files?.[0]; if (file) readFile(file); }); $('#syncRefresh').addEventListener('click', async () => { if (!(await pullLocalSnapshot())) $('#tecFile').click(); }); setInterval(pullLocalSnapshot, 15 * 60 * 1000);
$('#timerToggle').addEventListener('click', () => timerRunning ? pauseTimer('manual') : startTimer()); $('#hudTimerToggle').addEventListener('click', () => timerRunning ? pauseTimer('hud') : startTimer()); $('#hudSessionType').addEventListener('change', (event) => { $<HTMLSelectElement>('#sessionType').value = (event.target as HTMLSelectElement).value; }); $('#hudPanel').addEventListener('click', () => document.querySelector<HTMLButtonElement>('[data-view="dashboard"]')!.click()); $('#timerReset').addEventListener('click', () => { if (timerRunning) pauseTimer('reset'); state.daily[dayKey()] = { seconds: 0, sessions: [] }; saveState(); render(); });
['visibilitychange', 'blur'].forEach((eventName) => document.addEventListener(eventName, () => { if (timerRunning) { timerLoop(); renderTimer(); } })); window.addEventListener('focus', () => { if (timerRunning) timerLast = Date.now(); renderTimer(); }); window.addEventListener('beforeunload', () => { if (timerRunning) pauseTimer('unload'); });
...
window.addEventListener('message', (event: MessageEvent<{ type?: string; playing?: boolean; player?: string; code?: string; seconds?: number; completed?: boolean; catalog?: ContentItem[] }>) => {
  ...
  if (event.data?.type === 'dataprev-study-state') { if (event.data.playing && !timerRunning) startTimer(); if (!event.data.playing && timerRunning) pauseTimer('player'); return; }
