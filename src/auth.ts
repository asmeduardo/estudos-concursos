...
async function requestAuthEmail(action: 'signup' | 'resend_signup' | 'recovery', email: string, details: { password?: string; name?: string; redirectTo: string }): Promise<void> {
...
function renderSignup(): void {
  ...
  form.addEventListener('submit', async (event) => { event.preventDefault(); const data = new FormData(form), email = String(data.get('email')).trim(), password = String(data.get('password')), validation = validPassword(password), button = form.querySelector<HTMLButtonElement>('button[type="submit"]')!; if (validation) { notice(validation, 'error'); return; } buttonBusy(button, true, 'Enviando código…'); try { await requestAuthEmail('signup', email, { password, name: String(data.get('name')).trim(), redirectTo: url(`cadastro.html?verify=1&email=${encodeURIComponent(email)}`) }); startCooldown('signup', email); location.replace(url(`cadastro.html?verify=1&email=${encodeURIComponent(email)}`)); } catch { notice('Não foi possível criar a conta agora. Tente novamente em instantes.', 'error'); buttonBusy(button, false, 'Criar conta'); } });
}
