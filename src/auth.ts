...
const appWindow = window as AuthWindow;
...
async function requestAuthEmail(action: 'signup' | 'signup_otp' | 'resend_signup' | 'recovery', email: string, details: { password?: string; name?: string; redirectTo: string }): Promise<void> {
  ...
  try { const body = await result.error?.context?.json(); if (body?.error) message = String(body.error); } catch { /* resposta genérica intencional */ }
...
function renderSignup(): void {
  shell(`<h2>Crie sua conta</h2><p class="lead">Comece informando seu e-mail. Enviaremos um código para confirmar que ele é seu.</p><form id="signupEmailForm"><label>E-mail<input name="email" type="email" autocomplete="email" required></label><button class="primary" type="submit">Cadastrar</button></form><p id="notice" class="notice" role="status"></p><div class="auth-links"><span>Já tem uma conta? <a href="auth.html">Entrar</a></span></div>`);
  const form = document.querySelector<HTMLFormElement>('#signupEmailForm')!;
  form.addEventListener('submit', async (event) => { event.preventDefault(); const email = String(new FormData(form).get('email')).trim(), button = form.querySelector<HTMLButtonElement>('button[type="submit"]')!; if (cooldownRemaining('signup', email)) { notice(`Aguarde ${cooldownRemaining('signup', email)}s para solicitar outro código.`, 'error'); return; } buttonBusy(button, true, 'Enviando código…'); try { await requestAuthEmail('signup_otp', email, { redirectTo: url(`cadastro.html?verify=1&email=${encodeURIComponent(email)}`) }); startCooldown('signup', email); location.replace(url(`cadastro.html?verify=1&email=${encodeURIComponent(email)}`)); } catch { notice('Não foi possível enviar o código agora. Tente novamente em instantes.', 'error'); buttonBusy(button, false, 'Cadastrar'); } });
}
function renderVerify(): void {
  const email = query.get('email') || ''; if (!email) { location.replace(url('cadastro.html')); return; }
  shell(`<h2>Confirme seu e-mail</h2><p class="lead">Enviamos um código de 6 dígitos para <strong>${email.replace(/[<>&"]/g, '')}</strong>.</p><form id="verifyForm"><label>Código de confirmação<span class="otp-group">${[0,1,2,3,4,5].map((index) => `<input class="otp-digit" data-otp="${index}" inputmode="numeric" autocomplete="one-time-code" maxlength="1" pattern="[0-9]" aria-label="Dígito ${index + 1}" required>`).join('')}</span></label><button class="primary" type="submit">Validar e continuar</button></form><p id="notice" class="notice" role="status"></p><p class="cooldown"><button class="password-toggle" id="resendCode" type="button">Reenviar código</button></p><div class="auth-links"><a href="cadastro.html">Usar outro e-mail</a></div>`);
  const digits = [...document.querySelectorAll<HTMLInputElement>('[data-otp]')];
  digits.forEach((input, index) => input.addEventListener('input', () => { input.value = input.value.replace(/\D/g, '').slice(-1); if (input.value && digits[index + 1]) digits[index + 1].focus(); }));
  digits.forEach((input, index) => input.addEventListener('keydown', (event) => { if (event.key === 'Backspace' && !input.value && digits[index - 1]) digits[index - 1].focus(); }));
  digits[0]?.addEventListener('paste', (event) => { event.preventDefault(); const text = event.clipboardData?.getData('text').replace(/\D/g, '').slice(0, 6) || ''; digits.forEach((digit, index) => { digit.value = text[index] || ''; }); digits[Math.min(text.length, 5)]?.focus(); });
  bindCooldown(document.querySelector<HTMLButtonElement>('#resendCode')!, 'signup', email, () => requestAuthEmail('resend_signup', email, { redirectTo: location.href }));
  const form = document.querySelector<HTMLFormElement>('#verifyForm')!;
  form.addEventListener('submit', async (event) => { event.preventDefault(); const button = form.querySelector<HTMLButtonElement>('button[type="submit"]')!, token = digits.map((digit) => digit.value).join(''); if (token.length !== 6) { notice('Informe os 6 dígitos do código.', 'error'); return; } buttonBusy(button, true, 'Validando…'); const result = await auth!.auth.verifyOtp({ email, token, type: 'email' }); if (result.error) { notice('Código inválido ou expirado. Solicite um novo código.', 'error'); buttonBusy(button, false, 'Validar e continuar'); return; } renderSignupDetails(email); });
}
function renderSignupDetails(email: string): void {
  shell(`<h2>E-mail confirmado</h2><p class="lead">Agora conclua seu cadastro para entrar no Nexame.</p><form id="signupDetailsForm"><label>Nome<input name="name" autocomplete="name" required></label>${passwordField('Crie uma senha', 'password', 'new-password')}<p class="small">Mínimo de 8 caracteres, com maiúscula, minúscula e número.</p><button class="primary" type="submit">Concluir cadastro</button></form><p id="notice" class="notice" role="status"></p>`); bindPasswordToggles();
  const form = document.querySelector<HTMLFormElement>('#signupDetailsForm')!;
  form.addEventListener('submit', async (event) => { event.preventDefault(); const data = new FormData(form), password = String(data.get('password')), validation = validPassword(password), button = form.querySelector<HTMLButtonElement>('button[type="submit"]')!; if (validation) { notice(validation, 'error'); return; } if (!auth) return; buttonBusy(button, true, 'Concluindo…'); const result = await auth.auth.updateUser({ password, data: { display_name: String(data.get('name')).trim() } }); if (result.error) { const message = result.error.status === 422 || result.error.code === 'weak_password' ? 'Essa senha foi recusada pelo Supabase. Use uma senha forte e diferente de outras senhas já usadas.' : result.error.message || 'Não foi possível concluir o cadastro. Tente novamente.'; notice(message, 'error'); buttonBusy(button, false, 'Concluir cadastro'); return; } location.replace(safeNext()); });
}
function renderRecovery(): void {
  ...
  const requestRecovery = async (email: string): Promise<void> => requestAuthEmail('recovery', email, { redirectTo: url('redefinir-senha.html') });
...
async function boot(): Promise<void> {
  ...
  if (mode === 'verify' && session) { renderSignupDetails(query.get('email') || session.user.email || ''); return; }
