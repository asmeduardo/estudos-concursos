...
function renderSignupDetails(email: string): void {
  ...
  form.addEventListener('submit', async (event) => { event.preventDefault(); const data = new FormData(form), password = String(data.get('password')), validation = validPassword(password), button = form.querySelector<HTMLButtonElement>('button[type="submit"]')!; if (validation) { notice(validation, 'error'); return; } if (!auth) return; buttonBusy(button, true, 'Concluindo…'); const result = await auth.auth.updateUser({ password, data: { display_name: String(data.get('name')).trim() } }); if (result.error) { notice('Não foi possível concluir o cadastro. Tente novamente.', 'error'); buttonBusy(button, false, 'Concluir cadastro'); return; } location.replace(safeNext()); });
}
