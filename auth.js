...
function renderSignup() {
    ...
    const form = document.querySelector('#signupEmailForm');
    form.addEventListener('submit', async (event) => { event.preventDefault(); const email = String(new FormData(form).get('email')).trim(), button = form.querySelector('button[type="submit"]'); if (cooldownRemaining('signup', email)) {
        notice(`Aguarde ${cooldownRemaining('signup', email)}s para solicitar outro código.`, 'error');
        return;
    } buttonBusy(button, true, 'Enviando código…'); try {
        await requestAuthEmail('signup_otp', email, { redirectTo: url(`cadastro.html?verify=1&email=${encodeURIComponent(email)}`) });
        startCooldown('signup', email);
        location.replace(url(`cadastro.html?verify=1&email=${encodeURIComponent(email)}`));
    }
    catch {
        notice('Não foi possível enviar o código agora. Tente novamente em instantes.', 'error');
        buttonBusy(button, false, 'Cadastrar');
    } });
}
function renderVerify() {
    ...
    const digits = [...document.querySelectorAll('[data-otp]')];
    digits.forEach((input, index) => input.addEventListener('input', () => { input.value = input.value.replace(/\D/g, '').slice(-1); if (input.value && digits[index + 1])
        digits[index + 1].focus(); }));
    digits.forEach((input, index) => input.addEventListener('keydown', (event) => { if (event.key === 'Backspace' && !input.value && digits[index - 1])
        digits[index - 1].focus(); }));
    ...
    form.addEventListener('submit', async (event) => { event.preventDefault(); const button = form.querySelector('button[type="submit"]'), token = digits.map((digit) => digit.value).join(''); if (token.length !== 6) {
        notice('Informe os 6 dígitos do código.', 'error');
        return;
    } buttonBusy(button, true, 'Validando…'); const result = await auth.auth.verifyOtp({ email, token, type: 'email' }); if (result.error) {
        notice('Código inválido ou expirado. Solicite um novo código.', 'error');
        buttonBusy(button, false, 'Validar e continuar');
        return;
    } renderSignupDetails(email); });
}
function renderSignupDetails(email) {
    ...
    bindPasswordToggles();
    const form = document.querySelector('#signupDetailsForm');
    form.addEventListener('submit', async (event) => { event.preventDefault(); const data = new FormData(form), password = String(data.get('password')), validation = validPassword(password), button = form.querySelector('button[type="submit"]'); if (validation) {
        ...
        const message = result.error.status === 422 || result.error.code === 'weak_password' ? 'Escolha uma senha mais forte, com pelo menos 8 caracteres, letras maiúsculas, minúsculas e números.' : 'Não foi possível concluir o cadastro. Tente novamente.';
...
async function boot() {
    ...
    const session = (await auth.auth.getSession()).data.session;
    if (mode === 'verify' && session) {
        renderSignupDetails(query.get('email') || session.user.email || '');
        return;
    }
    if (session) {
        location.replace(safeNext());
