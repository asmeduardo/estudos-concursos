...
const DEFAULT_CONTEST = { id: 'dataprev-2026', name: 'Dataprev 2026 — Desenvolvedor de Software', examDate: '2026-10-11', targetMinutes: DEFAULT_TARGET, specificWeight: 2.5, generalWeight: 1, minQuestions: MIN_SAMPLE, updatedAt: new Date().toISOString() };
...
function renderHud(list = ranked()) {
    ...
    $('#hudTarget').textContent = `${state.targetMinutes} min`;
    $('#hudRecovery').textContent = String(recovery.length);
    $('#hudRecovery').style.color = recovery.length ? '#fb7185' : '#34d399';
    $('#hudNext').textContent = next ? next.name : 'Importar TEC';
    $('#hudNext').title = next ? next.d.action : 'Importe um snapshot do TEC';
...
function renderTable(list) { $('#cadernosEmpty').style.display = list.length ? 'none' : 'block'; $('#cadernosBody').innerHTML = list.map((x) => { const d = x.d, pct = x.attempted ? d.raw : 0; return `<tr><td><strong>${esc(x.name)}</strong><small>${esc(x.topic)}</small></td><td>${x.subject === 'specific' ? 'Específicas' : 'Gerais'}</td><td><div>${pct.toFixed(1)}%</div><div class="bar"><span class="${d.status}" style="width:${Math.min(100, pct)}%"></span></div></td><td>${x.correct}/${x.attempted}<small>${x.repeatErrors || 0} erros repetidos</small></td><td><span class="badge ${d.status}">${d.eligible ? (d.raw < 80 ? 'RECUPERAÇÃO' : d.raw < 85 ? 'ATENÇÃO' : 'OK') : 'AMOSTRA PEQUENA'}</span></td><td>${esc(d.action)}</td></tr>`; }).join(''); }
function renderNext(x) { if (!x) {
    $('#planStatus').textContent = 'Sem diagnóstico';
    $('#planStatus').className = 'badge neutral';
    $('#nextBlock').innerHTML = '<span class="badge neutral">AGUARDANDO</span><div><strong>Importe os resultados do TEC ou cadastre um caderno.</strong><div class="muted">O motor priorizará a recuperação com maior retorno por minuto.</div></div>';
    return;
} const d = x.d, contest = activeContest(), title = d.raw < 80 ? 'Recuperação obrigatória' : d.raw < 85 ? 'Manutenção reforçada' : 'Revisão espaçada'; $('#planStatus').textContent = title; $('#planStatus').className = `badge ${d.status}`; $('#nextBlock').innerHTML = `<span class="badge ${d.status}">${x.subject === 'specific' ? `ESPECÍFICAS · ${contest.specificWeight}` : `GERAIS · ${contest.generalWeight}`}</span><div><strong>${esc(x.name)}</strong><div>${esc(d.action)} · ${x.attempted} questões · <b>${d.raw.toFixed(1)}%</b></div><div class="muted">Prioridade ${d.risk.toFixed(2)} · ${esc(x.topic || 'assunto do caderno')}</div></div>`; }
